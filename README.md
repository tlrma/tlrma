# 안녕하세요, 임베디드와 AI를 연결하는 개발자입니다 👋

<!-- 아래 한줄 소개를 자유롭게 수정하세요 -->
> 한줄 소개: 여기에 원하는 소개 문장을 입력하세요.

<div align="center">

[![Portfolio](https://img.shields.io/badge/Portfolio-my--portfolio-181717?style=for-the-badge&logo=github)](https://github.com/tlrma/my-portfolio)
[![Email](https://img.shields.io/badge/onono1141@gmail.com_here-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:onono1141@gmail.com)
[![Status](https://img.shields.io/badge/Currently-Learning%20and%20Building-0A66C2?style=for-the-badge)](#)

</div>

---

## About Me

- 임베디드 시스템과 AI를 결합한 프로젝트에 관심이 있습니다.
- 문제를 직접 구현하고, 성능 저하 원인을 분석해 개선하는 과정을 중요하게 생각합니다.
- 센서 기반 제어, computer vision, ROS, 웹 서비스 구현 경험을 쌓아 왔습니다.

---

## Tech Stack

<div align="center">

[![My Skills](https://skillicons.dev/icons?i=c,cpp,python,linux,flask,sqlite)](https://skillicons.dev)

<br>
<br>

![ROS](https://img.shields.io/badge/ROS-22314E?style=for-the-badge&logo=ros&logoColor=white)

</div>

---

## Featured Projects

<table>
<tr>
<td valign="top" width="50%">

### 🛴 안전한 전동 킥보드 시스템

ATmega128 기반으로 헬멧 착용, 음주 여부, 양손 핸들링을 확인한 뒤에만 작동하는 안전 중심 킥보드 시스템입니다.

**Tech**  
![ATmega128](https://img.shields.io/badge/ATmega128-5C3EE8?style=flat-square)
![MQ3](https://img.shields.io/badge/MQ3%20Sensor-0A66C2?style=flat-square)
![Touch](https://img.shields.io/badge/Touch%20Sensor-2EA44F?style=flat-square)
![UART](https://img.shields.io/badge/UART-6F42C1?style=flat-square)

</td>
<td valign="top" width="50%">

### 🚁 타겟 드론 Following 시스템

전면 카메라 영상에서 타겟 드론을 감지하고, bounding box 기반으로 위치를 조정하는 ROS 기반 드론 제어 시스템입니다.

**Tech**  
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![ROS](https://img.shields.io/badge/ROS-22314E?style=flat-square&logo=ros&logoColor=white)
![Detection](https://img.shields.io/badge/Object%20Detection-DB4437?style=flat-square)
![Jetson](https://img.shields.io/badge/Jetson%20Nano-76B900?style=flat-square&logo=nvidia&logoColor=white)

</td>
</tr>
<tr>
<td valign="top" width="50%">

### 🎵 Facial Emotion Detect 노래 추천 웹사이트

표정 인식과 사용자 식별을 결합해 감정에 맞는 음악을 추천하는 웹 서비스입니다.

**Tech**  
![ResNet18](https://img.shields.io/badge/ResNet18-1F6FEB?style=flat-square)
![KNN](https://img.shields.io/badge/K--NN-8250DF?style=flat-square)
![Flask](https://img.shields.io/badge/Flask-000000?style=flat-square&logo=flask&logoColor=white)
![SQLite3](https://img.shields.io/badge/SQLite3-003B57?style=flat-square&logo=sqlite&logoColor=white)
![Spotify](https://img.shields.io/badge/Spotify%20API-1DB954?style=flat-square&logo=spotify&logoColor=white)

</td>
<td valign="top" width="50%">

### 📂 More

더 자세한 프로젝트 설명과 활동 정리는 아래 저장소에서 볼 수 있습니다.

**Open Repository**  
[![Go to Portfolio Repo](https://img.shields.io/badge/Open-Detailed%20Portfolio-181717?style=for-the-badge&logo=github)](https://github.com/tlrma/my-portfolio)

</td>
</tr>
</table>

---

## Activities
- **SAMSUNG SW AI ACADEMY FOR YOUTH 15기** | 2026.01 ~
- **NCOSS 서포터즈** | 2024.03 ~ 2024.08
- **한국장학재단 대학생 재능봉사 캠프** | 2024.02.12 ~ 2024.02.15
- **소모임 C언어 세미나** | 2021.09 ~ 2023.12

---

## Contact

- Email: `onono1141@gmail.com`
- GitHub: `https://github.com/tlrma`

<!-- 필요하면 아래 항목을 추가하세요 -->
<!-- Blog: -->
<!-- Notion: -->
